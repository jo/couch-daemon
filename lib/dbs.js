...
module.exports = function(url, options) {
  ...
  var stream = _.pipeline(
    ...
    _.filter(function(d) {
      // Whitelist all if none particular is mentionen
      if (!options.whitelist && !options.whitelist_regexp) {
          return true;
      }
      // Whilelist regexp matching dbs
      if (options.whitelist_regexp && options.whitelist_regexp.test(d.db_name)) {
        return true;
      }
      // Whitelist all in... well, whitelist
      return options.whitelist && options.whitelist.indexOf(d.db_name) > -1;
    }),

    _.reject(function(d) {
      if (options.blacklist_regexp && options.blacklist_regexp.test(d.db_name)) {
        return true;
      }
