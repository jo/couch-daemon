...
module.exports = function(defaults, worker) {
  ...
  defaults.version = defaults.version || pkg.version;
  ...
  function pipeline(options) {
    ...
    if (options.version) {
      console.log(defaults.name, defaults.version);
      process.exit(0);
    }
    ...
    if (typeof options.whitelist_regexp === 'string' && options.whitelist_regexp.match(/^\/.*\/$/) ) {
      options.whitelist_regexp = new RegExp(options.whitelist_regexp.substring(1, options.whitelist_regexp.length - 1));
    }
    ...
    if (typeof options.blacklist_regexp === 'string' && options.blacklist_regexp.match(/^\/.*\/$/) ) {
      options.blacklist_regexp = new RegExp(options.blacklist_regexp.substring(1, options.blacklist_regexp.length - 1));
    }
